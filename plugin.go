...
func (d *SampleDatasource) query(_ context.Context, pCtx backend.PluginContext, query backend.DataQuery) backend.DataResponse {
	...
	frame.Fields = append(frame.Fields,
		...
		data.NewField("values", nil, []float64{10, 10 + slope * 10}),		//todo 这里的值可以通过斜率去控制
	)
	...
	frame.Fields = append(frame.Fields,
		data.NewField("time", nil, []time.Time{query.TimeRange.From, query.TimeRange.To}),
		data.NewField("values", nil, []float64{10, 10 + slope2 * 10}),		//todo 这里的值可以通过斜率去控制
	)
	...
	response.Frames = append(response.Frames, frame, frame2)
